...
public sealed class DominioServiceTests
{
    ...
    public void Registry_IsExplicitCompleteAndInternallyConsistent()
    {
        ...
        Assert.Equal(34, MasterCatalogRegistry.Catalogs.Count);
        Assert.Equal(34, MasterCatalogRegistry.Catalogs.Select(catalog => catalog.Code).Distinct().Count());
        Assert.Equal(34, MasterCatalogRegistry.Catalogs.Select(catalog => catalog.Route).Distinct().Count());
